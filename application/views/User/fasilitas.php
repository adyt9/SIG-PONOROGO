                ...
                <?php foreach ($data_fasilitas as $row) : ?>
                    <div class="col-lg-6 col-md-12 col-sm-6">
                        <div class="img_wrapper">
                            ...
                            <div class="tools_i">
                                <div class="directions_list_map" onclick="onHtmlClick('<?= $row->longitude ?>','<?= $row->latitude ?>','<?= $row->nama_fasilitas ?>','<?= $row->deskripsi ?>','<?= $row->kontak ?>','<?= $row->gambar_fasilitas ?>','<?= $row->alamat ?>');return false;">
                            ...
                            <div class="img_container">
                                <a href="<?= site_url('User/detail_agenda/') . $row->id_fasilitas; ?>">
...
<script>
    ...
    cordx.forEach(function(index) {
        test.push(JSON.parse('{"type": "Feature", "geometry": {"type": "Point", "coordinates": [' + index.kordinat_lng + ',' + index.kordinat_lat + ']},"properties": {"title": "' + index.nama_wisata + '","description": "<p>' + index.deskripsi + '</p>","icon":"' + index.icon + '"}}'));
    });
    ...
    function createPopUp(a, b, nama_wisata, deskripsi, kontak, image, alamat) {
        ...
        var popup = new mapboxgl.Popup({
            ...
            .setHTML('<div id=\'marker-info\' class=\'marker-info\'>' + '<img style=\'width:100%;border-top-left-radius:7px;border-top-right-radius:7px;\' src="<?= base_url('uploads/') ?>' + image + '" alt=""><span>' + '<h3>' + nama_wisata + '</h3>' +
                '<em>' + deskripsi + '</em><strong>' + alamat + '</strong><p style=\'margin : 5px;\'><a href="tel://' + kontak + '" class="btn_infobox_phone">+' + kontak + '</a></p><span></div>')
            .addTo(map);
    }
